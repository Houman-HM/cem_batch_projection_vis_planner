...
class batch_occ_tracking():
	...
	def __init__(self, P, Pdot, Pddot, v_max, a_max, t_fin, num, num_batch_projection,
		...
		self.min_dist = 1
		self.max_dist = 3
		self.tracking_weight = 100
		self.d_avg_target = (self.min_dist + self.max_dist)/2
	...
	@partial(jit, static_argnums = (0,))
	def compute_projection(self, x_samples_init, y_samples_init, b_x_eq, b_y_eq, x_target, y_target, alpha_a, d_a, alpha_v, d_v, lamda_x, lamda_y, x_workspace, y_workspace, alpha_workspace, d_workspace, c_x_samples_init, c_y_samples_init):
		
		cost = jnp.identity(self.nvar)
		...
		lincost_x = -c_x_samples_init
		lincost_y = -c_y_samples_init
	...
	def compute_projection_samples(self, x_init, vx_init, ax_init, y_init, vy_init, ay_init, alpha_a,
		...
		for i in range(0, self.maxiter_projection):
			...
			c_x_samples, c_y_samples, x_samples, y_samples, xdot_samples, \
				...
				y_target, alpha_a, d_a, alpha_v, d_v, lamda_x, 
		...
		return x_samples, y_samples, xdot_samples, ydot_samples, xddot_samples, \
			yddot_samples, c_x_samples, c_y_samples

	@partial(jit, static_argnums=(0,))	

	def compute_cost_batch(self, x_samples, y_samples, xdot_samples, ydot_samples, xddot_samples, yddot_samples, x_target, y_target, obstacle_points): 
		...
		velocity_cost = jnp.linalg.norm(jnp.maximum(0, jnp.sqrt(xdot_samples**2 + ydot_samples**2) - self.v_max), axis=1)
		acc_cost = jnp.linalg.norm(jnp.maximum(0, jnp.sqrt(xddot_samples**2 + yddot_samples**2) - self.a_max), axis=1)
		min_dist_cost = jnp.linalg.norm(jnp.maximum(0, -(x_samples - x_target)**2 - (y_samples - y_target)**2 + self.min_dist**2), axis=1)
		max_dis_cost = jnp.linalg.norm(jnp.maximum(0, (x_samples - x_target)**2  + (y_samples - y_target)**2 - self.max_dist**2), axis=1)
		distance_cost= min_dist_cost + max_dis_cost

		target_dist = ((x_samples-x_target)**2+(y_samples-y_target)**2- self.d_avg_target**2)
		total_cost = velocity_cost * self.velocity_weight + acc_cost * self.acc_weight + distance_cost * self.tracking_weight + 0.9 * jnp.linalg.norm(target_dist, axis =1)**2 + \
							cost_smoothness * self.smoothness_weight + occlusion_cost * self.occlusion_weight
		return total_cost
	...
	def compute_cem(self, key, x_init, vx_init, ax_init, y_init, vy_init, ay_init, alpha_a, d_a, alpha_v, d_v,
		...
		for i in range(0, self.maxiter_cem):
			...
			x_samples, y_samples, xdot_samples, ydot_samples, xddot_samples, yddot_samples, c_x_samples, c_y_samples = self.compute_projection_samples(x_init, vx_init, ax_init, y_init, vy_init, ay_init
																								, alpha_a, d_a, alpha_v, d_v, x_target, y_target, lamda_x, 
																								 lamda_y, x_samples_init, y_samples_init, x_workspace, y_workspace, 
																								 alpha_workspace, d_workspace, c_x_samples_init, c_y_samples_init, vx_target, vy_target)
			c_x_ellite_projection = c_x_samples
			c_y_ellite_projection = c_y_samples

			x_ellite_projection = x_samples
			y_ellite_projection = y_samples

			xdot_ellite_projection = xdot_samples
			ydot_ellite_projection = ydot_samples

			xddot_ellite_projection = xddot_samples
			yddot_ellite_projection = yddot_samples

			total_cost = self.compute_cost_batch(x_ellite_projection, y_ellite_projection, xdot_ellite_projection, ydot_ellite_projection,
								 xddot_ellite_projection, yddot_ellite_projection, x_target, y_target, obstacle_points)
			...
			c_x_ellite = c_x_samples[idx[0:ellite_num]]
			c_y_ellite = c_y_samples[idx[0:ellite_num]]
			
			x_ellite = x_samples[idx[0:ellite_num]]
			y_ellite = y_samples[idx[0:ellite_num]]
			...
			x_samples_shift =  x_ellite
			y_samples_shift = y_ellite

		alpha_target = 0
		d_target = 0
		return 	c_x_best, c_y_best, total_cost[idx_min], x_samples[idx_min], y_samples[idx_min], alpha_v, d_v, alpha_a, d_a, alpha_target, d_target, lamda_x, lamda_y, alpha_workspace, d_workspace, key, x_samples_shift, y_samples_shift
